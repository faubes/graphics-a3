...
void main() {
  ...
  for (int i=0; i < nLights; i++) {
      ...
      float distanceLight = length(lightFrag[i].xyz);
      
      float attenuation = 1.0;
      if (lightFrag[i].w != 0.0) {
         attenuation /= (lights[i].constant_attenuation +
             lights[i].linear_attenuation * distanceLight +
             lights[i].quadratic_attenuation * distanceLight * distanceLight);
      }

      // ambient term
      vec4 ambient = materials[mI].ambient * lights[i].ambient;

      // diffuse term
      float dotNL = max(0.0,dot(NVec,LVec));
      vec4 diffuse = materials[mI].diffuse * lights[i].diffuse * dotNL;

      vec3 HVec = normalize(LVec+EVec);
      float dotNH = max(0.0,dot(NVec,HVec));
      vec4 specular = materials[mI].specular * lights[i].specular 
           * pow(dotNH,materials[mI].shininess);

      // spot light Assume greater than 90 means not a spotlight
      // lights[index].spotCutoff <= 90.0 // spotlight?
      ...
      if ( lights[i].spot_cutoff < 180.0 ) {
          ...
          if ( dotSV < cos(radians(lights[i].spot_cutoff))) {
            ...
            spot_attenuation = pow(dotSV,lights[i].spot_exponent);
        }
      }
        totalcolor += ambient + attenuation * spot_attenuation * (diffuse + specular);
  }
  // color
  color = clamp(totalcolor, 0.0, 1.0);
  // color = colorVertFrag;
  // color = vec4(LVec,1.0);
}
